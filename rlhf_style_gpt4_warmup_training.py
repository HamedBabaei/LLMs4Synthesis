...
from scripts.reward import GPT4FeaturesRewardModel
...
def build_dataset(dataset, tokenizer, input_max_text_length):
    def tokenize(sample):
        ...
        sample["input_ids"] = tokenizer.apply_chat_template(messages, temperature=0,
                                                            return_tensors="pt")[0][: input_max_text_length]
...
if __name__ == "__main__":
    ...
    print("args.rlhf_style_gpt4_warmup_start_model:", args.rlhf_style_gpt4_warmup_start_model)
    print("args.rlhf_style_gpt4_warmup_optimizer_name:", args.rlhf_style_gpt4_warmup_optimizer_name)
    print("args.base_model_id:", args.base_model_id)
    print("args.rlhf_style_gpt4_warmup_dir:", args.rlhf_style_gpt4_warmup_dir)
    print("---" * 30)
    ...
    train_data = dataset_builder.orkg_synthesis_rlhf_gpt4()
    print("datase size:", len(train_data))

    reward_model = GPT4FeaturesRewardModel(args=args)
    ...
    model = AutoModelForCausalLMWithValueHead.from_pretrained(
        args.rlhf_style_gpt4_warmup_start_model,
        load_in_4bit=True,
    ...
    config = PPOConfig(
        model_name=args.rlhf_style_gpt4_warmup_optimizer_name,
        ...
        project_kwargs={'logging_dir': args.rlhf_style_gpt4_warmup_dir},
        cliprange=0.3
    )
    ...
    for epoch in range(args.rlhf_num_train_epochs):
        for batch_no, batch in tqdm(enumerate(dataset)):
            ...
            response = ppo_trainer.generate(query_tensors, return_prompt=False, **generation_kwargs)
            # print("Generation size:", len(response[0]))
            response_tensors.append(response.squeeze())

            batch["response"] = [tokenizer.decode(r.squeeze()) for r in response_tensors]

            reward_estimation = reward_model.get_reward(batch["response"][0], batch)

            rewards = [torch.tensor(reward_estimation['reward'])]
            rewards_lst.append(reward_estimation['reward'])

            print(f"REWARD: {rewards} -- AVG:{sum(rewards_lst)/len(rewards_lst)} -- Len:{reward_estimation['wc']}")
            ...
            torch.cuda.empty_cache()
            # dataset[batch_no][f'epoch-{epoch}'] = reward_estimation
            eval_loger.append({
                "batch": {key: val for key, val in batch.items() if key != 'input_ids'},
                "reward": reward_estimation
            })
            io.write_json(args.rlhf_style_gpt4_warmup_eval_loger, eval_loger)
            if train_step % 100 == 0:
                print("Run ppo_trainer.save_pretrained!")
                ppo_trainer.save_pretrained(os.path.join(args.rlhf_style_gpt4_warmup_dir, f"step-{str(train_step)}"))
            train_step += 1
            print("--" * 40)

        print("Run ppo_trainer.save_pretrained!")
        ppo_trainer.save_pretrained(os.path.join(args.rlhf_style_gpt4_warmup_dir, f"epoch-{str(epoch)}"))

    ppo_trainer.save_pretrained(args.rlhf_style_gpt4_warmup_dir)
