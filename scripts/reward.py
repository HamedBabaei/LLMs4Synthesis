...
class StylisticRewardModel:
    ...
    def build_reward_model(self) -> Any:
        ...
        self.reward_model = Pipeline (
            steps=[
                ("Stylistic Representation", FeatureUnion(
                    transformer_list=[
                        ('Paper Format Detector', vectorizer),
    ...
    def get_reward(self, query_response, query) -> float:
        ...
        while True:
            try:
                ...
                print("ERROR while calling GPT4-API in REWARD MODELING!")
...
#         basic_wcs, basic_ps = self.baseic_reward_model.get_reward_for_gpt4(query_response)
        
#         # exceeds the word limit then it is a Conciseness issue 
#         sum_scores = sum(scores) - int(response['Conciseness']['rating']) +  basic_wcs

#         # if it follows a paper structure then it is a Readability issue 
#         if basic_ps == 1:
#             sum_scores = sum_scores - int(response['Readability']['rating']) -  basic_ps

#         average = sum_scores/len(scores)

#         if average < 1.5:
#             gpt4_reward = average - 5 - 6
#         elif average < 2:
#             gpt4_reward = average - 5 - 5
#         elif average < 2.5:
#             gpt4_reward = average - 5 - 4
#         elif average < 3:
#             gpt4_reward = average - 5 - 3
#         elif average < 3.5:
#             gpt4_reward = average - 5 - 2
#         elif average < 4:
#             gpt4_reward = average - 5 - 1
#         elif average < 4.5:
#             gpt4_reward = average + 2
#         else:
#             gpt4_reward = average + 3
...
#             gpt4_reward = average - 5 - 1
#         elif average < 4:
#             gpt4_reward = average + 1
#         elif average < 4.5:
#             gpt4_reward = average + 2
#         else:
#             gpt4_reward = average + 3

#         basic_reward = self.baseic_reward_model.get_reward(query_response)


#         if gpt4_reward < 0 and basic_reward < 0:
#             reward = gpt4_reward + basic_reward
#         elif gpt4_reward < 0 and basic_reward >= 0:
#             reward = gpt4_reward
#         elif gpt4_reward > 0 and basic_reward < 0:
#             reward = (sum(scores) - int(response['Conciseness']['rating']))/len(scores) 
#             reward = reward * basic_reward
#         else:
#             reward = gpt4_reward * basic_reward

#         return {
#             "reward-gpt4": gpt4_reward,
#             "reward-basic": basic_reward,
#             "reward": reward,
#             "message": message,
#             "response": response, 
#             "completion-tokens": completion_tokens, 
