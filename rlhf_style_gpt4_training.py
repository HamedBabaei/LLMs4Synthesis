...
if __name__ == "__main__":
    ...
    print("args.rlhf_style_gpt4_start_model:", args.rlhf_style_gpt4_start_model)
    print("args.rlhf_style_gpt4_optimizer_name:", args.rlhf_style_gpt4_optimizer_name)
    print("args.base_model_id:", args.base_model_id)
    print("args.rlhf_style_gpt4_dir:", args.rlhf_style_gpt4_dir)
    print("---"*30)
    ...
    model = AutoModelForCausalLMWithValueHead.from_pretrained(
        args.rlhf_style_gpt4_start_model, 
        load_in_4bit=True,
    ...
    config = PPOConfig(
        model_name=args.rlhf_style_gpt4_optimizer_name,
        ...
        project_kwargs={'logging_dir':args.rlhf_style_gpt4_dir},
        cliprange=0.3
    )
    ...
    for epoch in range(args.rlhf_num_train_epochs):
        for batch_no, batch in tqdm(enumerate(dataset)):
            ...
            io.write_json(args.rlhf_style_gpt4_eval_loger, eval_loger)
            if train_step%100 == 0:
                print("Run ppo_trainer.save_pretrained!")
                ppo_trainer.save_pretrained(os.path.join(args.rlhf_style_gpt4_dir, f"step-{str(train_step)}"))
            train_step += 1

            print("--"*40)
            
        print("Run ppo_trainer.save_pretrained!")
        ppo_trainer.save_pretrained(os.path.join(args.rlhf_style_gpt4_dir, f"epoch-{str(epoch)}"))
        
    ppo_trainer.save_pretrained(args.rlhf_style_gpt4_dir)